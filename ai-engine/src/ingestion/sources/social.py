...
class SocialMediaDataSource(BaseDataSource):
    ...
    def get_metadata(self) -> SourceMetadata:
        return SourceMetadata(
            ...
            supported_fields=[
                "timestamp", "platform", "post_id", "author_id", "text",
                "sentiment_score", "sentiment_label", "matched_query_term",
                "retweet_count", "like_count", "reply_count", "language",
                "urgency_score", "explanation", "keywords"
            ],
            tags=["social", "twitter", "nlp", "sentiment", "signal", "complaints", "urgency", "explainable"],
        )
    ...
    async def fetch(self) -> IngestionResult:
        ...
        sample_tweets = [
            ("Major power outage in Sector 4! The electricity grid is completely down. Angry residents stuck in elevators!", "alex_systems"),
            ("Complete gridlock on I-95. A terrible traffic jam is stretching for 5 miles. Avoid!", "traffic_eye"),
            ("Water leakage alert! A massive burst pipe is causing flooding on Main Street.", "city_watcher_01"),
            ("Wifi failed again! The network down signal is completely dead, no internet in the whole office!", "commuter_daily"),
            ("Fallen tree blocking Broadway! Huge pothole is causing damaged railings and traffic light broken. Extremely dangerous!", "city_watcher_02"),
            ("Electricity restored in North Gate! Power outage is resolved. Thankfully, service is back.", "sector4_updates"),
            ("Traffic congestion cleared near I-405, things are looking better.", "commuter_la"),
            ("No internet connection at Sector 9, slow connection is making me angry. Wifi failed!", "wifi_user"),
            ("Burst pipe fixed at Sector 9, thankfully the flooding has stopped.", "plumber_pro"),
            ("Street light out on 5th Ave. Pothole makes it unsafe and terrible to drive at night!", "road_user")
        ]
        ...
        logger.info(f"[{meta.name}] Ingestion complete. Scraped and classified {len(records)} complaints.")
