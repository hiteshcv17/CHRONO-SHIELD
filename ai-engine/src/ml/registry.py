...
class ModelRegistryManager:
    ...
    def promote_model(self, model_id: str, tier: str) -> None:
        ...
        if model_id not in metadata["models"]:
            raise ValueError(f"Model ID '{model_id}' is not registered in the registry.")
        ...
        self._save_metadata(metadata)
        logger.info(f"Model '{model_id}' promoted to tier: {tier}. Checkpoint active: {tier_dest}")
    ...
    def rollback_tier(self, tier: str) -> None:
        ...
        if not current_active:
            raise ValueError(f"No active model currently promoted to tier '{tier}' to perform a rollback.")
        ...
        if not previous_active:
            raise ValueError(f"No previous active model found in history for tier '{tier}' to rollback to.")

        # Re-promote the previous model
        self.promote_model(previous_active, tier)
        logger.info(f"Tier '{tier}' rolled back from model '{current_active}' to previous model '{previous_active}'.")
