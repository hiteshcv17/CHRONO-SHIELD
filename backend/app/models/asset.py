...
class Asset(Base):
    ...
    id = Column(
        ...
        primary_key=True,
        index=True,
        default=lambda: f"ast-{uuid.uuid4().hex[:8]}",
    )
    ...
    status = Column(
        String(50), default="NOMINAL", nullable=False, index=True
    )  # NOMINAL, WARNING, CRITICAL, MAINTENANCE, DECOMMISSIONED
    region = Column(
        String(100), nullable=False, index=True
    )  # e.g., North Sector, Downtown Grid, etc.
    metadata_json = Column(
        String(4000), nullable=True
    )  # JSON-serialized metadata attributes
