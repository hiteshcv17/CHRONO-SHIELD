...
class NotificationChannelConfig(Base):
    ...
    id = Column(String(50), primary_key=True, index=True, default=lambda: f"cfg-{uuid.uuid4().hex[:8]}")
    channel_type = Column(String(20), unique=True, index=True, nullable=False)  # EMAIL, TELEGRAM, WEBHOOK
...
class NotificationDeliveryLog(Base):
    ...
    id = Column(String(50), primary_key=True, index=True, default=lambda: f"log-{uuid.uuid4().hex[:8]}")
