    ...
    def decorator(func: Callable):
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                if cached_val is not None:
                    ...
                    CACHE_OPERATIONS.labels(
                        prefix=prefix, operation="read", status="hit"
                    ).inc()
                    # Return the raw deserialized JSON
                    return json.loads(cached_val)
                else:
                    CACHE_OPERATIONS.labels(
                        prefix=prefix, operation="read", status="miss"
                    ).inc()
            except Exception as e:
                CACHE_OPERATIONS.labels(
                    prefix=prefix, operation="read", status="failure"
                ).inc()
            ...
            try:
                ...
                CACHE_OPERATIONS.labels(
                    prefix=prefix, operation="write", status="success"
                ).inc()
                logger.debug(
                    f"Cache MISS. Saved data to key: {cache_key} with TTL={ttl}s"
                )
            except Exception as e:
                CACHE_OPERATIONS.labels(
                    prefix=prefix, operation="write", status="failure"
                ).inc()
...
async def invalidate_cache_by_pattern(pattern: str) -> None:
    ...
    try:
        if hasattr(redis_client, "_store"):  # InMemoryTokenStore
            ...
            logger.info(
                f"Invalidated {len(keys_to_del)} local in-memory cache keys using pattern '{pattern}'"
            )
            ...
            while True:
                ...
                if cursor == 0:
                    break
            logger.info(
                f"Invalidated {keys_deleted} Redis cache keys using pattern '{pattern}'"
            )
        CACHE_OPERATIONS.labels(
            prefix=prefix, operation="invalidate", status="success"
        ).inc()
    except Exception as e:
        CACHE_OPERATIONS.labels(
            prefix=prefix, operation="invalidate", status="failure"
        ).inc()
