...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if settings.ENVIRONMENT == "production":
            logger.info(
                "Asserting database schema is managed and migrated by Alembic..."
            )
            ...
            async with async_session_factory() as session:
                try:
                    ...
                    version = res.scalar()
                    logger.info(
                        f"Database migrated via Alembic. Current Revision: {version}"
                    )
                except Exception as e:
                    logger.error(
                        "CRITICAL: Alembic schema version check failed! Ensure 'alembic upgrade head' was run."
                    )
                    raise RuntimeError(
                        "Database schema is not migrated or managed by Alembic!"
                    ) from e
        else:
        ...
        async with async_session_factory() as session:
            ...
            for c_type, cfg_data in [
                ...
                stmt_c = select(NotificationChannelConfig).where(
                    NotificationChannelConfig.channel_type == c_type
                )
                res_c = await session.execute(stmt_c)
                if not res_c.scalar_one_or_none():
                    new_cfg = NotificationChannelConfig(
                        channel_type=c_type, config=json.dumps(cfg_data), enabled=False
                    )
                    session.add(new_cfg)
            await session.commit()
            ...
            stmt_s = select(SystemSetting).where(
                SystemSetting.key == "rate_limiting_enabled"
            )
            ...
            if not res_s.scalar_one_or_none():
                ...
                logger.info(
                    "Default system setting 'rate_limiting_enabled' seeded to true."
                )
            ...
            if not res_r.scalars().first():
                now = datetime.utcnow()
                await ReportService.generate_executive_report(
                    session, "DAILY", now - timedelta(days=1), now
                )
                await ReportService.generate_executive_report(
                    session, "WEEKLY", now - timedelta(days=7), now
                )
                logger.info(
                    "Default daily and weekly executive reports generated and seeded."
                )
            ...
            if not res_a.scalars().first():
                ...
                for name, atype, status, region, metadata in [
                    (
                        "North Grid Substation Transformer T-01",
                        "TRANSFORMER",
                        "NOMINAL",
                        "North Sector",
                        {"capacity_kva": 2500, "voltage_kv": 13.8},
                    ),
                    (
                        "Main St Intersection Camera & Controller",
                        "TRAFFIC_ZONE",
                        "WARNING",
                        "Downtown Grid",
                        {"avg_daily_flow": 18200, "camera_status": "ONLINE"},
                    ),
                    (
                        "Central Trunk Aqueduct Conduit B",
                        "WATER_PIPELINE",
                        "CRITICAL",
                        "Central Hub",
                        {"pipe_diameter_in": 36, "flow_rate_lps": 480.5},
                    ),
                    (
                        "District 7 Warning Siren Tower",
                        "PUBLIC_SYSTEM",
                        "NOMINAL",
                        "South Sector",
                        {"decibel_output": 120, "coverage_radius_m": 800},
                    ),
                ]:
...
async def platform_health():
    ...
    try:
        ...
        services.append(
            ServiceHealth(name="postgresql", status="unavailable", detail=str(e)[:100])
        )
    ...
    try:
        ...
        services.append(
            ServiceHealth(name="redis", status="unavailable", detail=str(e)[:100])
        )
    ...
    try:
        ...
        async with async_session_factory() as session:
            stmt = select(
                ...
                PrioritizedAlertRecord.current_severity, PrioritizedAlertRecord.status
            )
            res = await session.execute(stmt)
...
app.include_router(
    correlation.router, prefix=f"{_V1}/correlation", tags=["Correlation Analytics"]
)
app.include_router(
    forecasting.router, prefix=f"{_V1}/forecasting", tags=["Predictive Forecasting"]
)
app.include_router(
    health.router, prefix=f"{_V1}/health", tags=["Infrastructure Health"]
)
...
app.include_router(
    benchmark.router, prefix=f"{_V1}/benchmark", tags=["Model Benchmarking"]
)
app.include_router(
    notification.router, prefix=f"{_V1}/notifications", tags=["Notification Delivery"]
)
app.include_router(
    report.router, prefix=f"{_V1}/reports", tags=["Infrastructure Reports"]
)
app.include_router(asset.router, prefix=f"{_V1}/assets", tags=["Infrastructure Assets"])
app.include_router(
    system_settings.router, prefix=f"{_V1}/settings", tags=["System Settings"]
)
