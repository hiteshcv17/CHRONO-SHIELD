...
async def get_geo_map(db: AsyncSession = Depends(get_db_session)):
    """
...
@router.get(
    "/regions", response_model=List[GeoRegionStatus], status_code=status.HTTP_200_OK
)
async def get_geo_regions(db: AsyncSession = Depends(get_db_session)):
    """
...
@router.get(
    "/anomaly-points",
    response_model=List[GeoAnomalyPoint],
...
async def get_geo_anomaly_points(
    ...
    category: Optional[str] = Query(
        None,
        description="Filter by infrastructure category: POWER, TRAFFIC, WATER, INTERNET, PUBLIC_INFRASTRUCTURE",
    ),
...
@router.get(
    "/heatmap", response_model=List[GeoHeatmapPoint], status_code=status.HTTP_200_OK
)
async def get_geo_heatmap(db: AsyncSession = Depends(get_db_session)):
    """
