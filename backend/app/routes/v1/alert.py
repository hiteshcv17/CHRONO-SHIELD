...
@router.get("/queue", response_model=PaginatedResponse[PrioritizedAlertResponse], status_code=status.HTTP_200_OK)
async def fetch_alert_queue(
    status_filter: Optional[str] = Query(None, description="Filter queue by status (ACTIVE, ACKNOWLEDGED, SUPPRESSED, ESCALATED, RESOLVED)"),
    severity: Optional[str] = Query(None, description="Filter queue by severity (CRITICAL, HIGH, MEDIUM, LOW)"),
    ...
    records, total = await AlertPrioritizationEngine.get_prioritized_queue(
        db, status_filter=status_filter, severity_filter=severity, page=page, page_size=page_size
    )
    return PaginatedResponse.build(items=records, total=total, page=page, page_size=page_size)


@router.post("/inject", response_model=ApiResponse[PrioritizedAlertResponse], status_code=status.HTTP_201_CREATED)
...
@router.put("/{alert_id}/acknowledge", response_model=ApiResponse[PrioritizedAlertResponse], status_code=status.HTTP_200_OK)
...
@router.put("/{alert_id}/resolve", response_model=ApiResponse[PrioritizedAlertResponse], status_code=status.HTTP_200_OK)
