...
class SettingsUpdate(BaseModel):
    rate_limiting_enabled: bool = Field(..., description="Whether to enforce API rate limiting policies")
...
async def get_system_settings(
    ...
    try:
        ...
        settings_dict = {}
        for s in settings:
            # Map string database representation back to native boolean for rate limiter
            if s.key == "rate_limiting_enabled":
                settings_dict[s.key] = s.value.lower() == "true"
            else:
                settings_dict[s.key] = s.value
                
        # Ensure default keys are returned if DB was clean
...
async def update_system_settings(
    ...
    """
    try:
        # 1. Update/insert rate_limiting_enabled in database
        ...
        # 2. Sync to Redis for high-performance middleware checks
        try:
