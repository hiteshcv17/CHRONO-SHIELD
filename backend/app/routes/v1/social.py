...
@router.get(
    "/complaints",
    response_model=List[SocialComplaintResponse],
    summary="Retrieve social media complaints feed",
)
async def get_social_complaints(
    ...
    try:
        ...
        logger.error(
            f"PostgreSQL fetch failed for complaints: {e}. Using mock fallback."
        )
...
@router.get(
    "/analytics",
    response_model=SocialAnalyticsResponse,
    summary="Retrieve social complaints analytics",
)
async def get_social_analytics(db: AsyncSession = Depends(get_db_session)):
    ...
    try:
        ...
        if count and count >= 6:
            ...
            cat_stmt = select(
                SocialComplaintRecord.category, func.count(SocialComplaintRecord.id)
            ).group_by(SocialComplaintRecord.category)
            cat_res = (await db.execute(cat_stmt)).all()
            cat_breakdown = [
                CategoryDistribution(category=row[0], count=row[1]) for row in cat_res
            ]

            sev_stmt = select(
                SocialComplaintRecord.severity, func.count(SocialComplaintRecord.id)
            ).group_by(SocialComplaintRecord.severity)
            sev_res = (await db.execute(sev_stmt)).all()
            sev_breakdown = [
                SeverityDistribution(severity=row[0], count=row[1]) for row in sev_res
            ]
        ...
        logger.error(
            f"PostgreSQL analytics aggregation failed: {e}. Using mock fallback."
        )
    ...
    return SocialAnalyticsResponse(
        ...
        severity_breakdown=[
            SeverityDistribution(severity=k, count=v) for k, v in sev_counts.items()
        ],
        clusters=clusters,
    )
...
async def trigger_social_ingestion(db: AsyncSession = Depends(get_db_session)):
    ...
    try:
        new_samples = [
            (
                "Catastrophic power outage in sector 2! Complete blackout, electricity grid is completely down!",
                "alex_systems",
                "blackout",
                "POWER",
                "CRITICAL",
                0.04,
                95.0,
                "Classified under POWER due to pattern 'blackout'. Sentiment is 0.04. Urgency scored at 95.0/100.",
                "power outage, blackout, electricity grid",
            ),
            (
                "Terrible traffic jam near the bridge. Heavy congestion stretches for miles. Bumper to bumper!",
                "commuter_daily",
                "traffic jam",
                "TRAFFIC",
                "WARNING",
                0.22,
                68.0,
                "Classified under TRAFFIC due to pattern 'traffic jam'. Sentiment is 0.22. Urgency scored at 68.0/100.",
                "traffic jam, heavy traffic, congestion",
            ),
            (
                "Massive water leakage at Sector 5 main pipeline. Severe flooding is reported. Burst pipe!",
                "city_watcher_01",
                "water leakage",
                "WATER",
                "CRITICAL",
                0.12,
                92.5,
                "Classified under WATER due to pattern 'water leakage'. Sentiment is 0.12. Urgency scored at 92.5/100.",
                "water leakage, burst pipe, flooding",
            ),
            (
                "Wifi failed again! The network down internet is completely dead. Slow connection!",
                "commuter_la",
                "wifi failed",
                "INTERNET",
                "CRITICAL",
                0.18,
                76.0,
                "Classified under INTERNET due to pattern 'wifi failed'. Sentiment is 0.18. Urgency scored at 76.0/100.",
                "wifi failed, network down, slow connection",
            ),
            (
                "Fallen tree blocking Broadway! Huge pothole is causing damaged railings.",
                "sector9_pro",
                "pothole",
                "PUBLIC_INFRASTRUCTURE",
                "CRITICAL",
                0.15,
                82.0,
                "Classified under PUBLIC_INFRASTRUCTURE due to pattern 'pothole'. Sentiment is 0.15. Urgency scored at 82.0/100.",
                "pothole, fallen tree, damaged railing",
            ),
        ]
