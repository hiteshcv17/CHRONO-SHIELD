...
class WeatherRecord(BaseModel):
    ...
    wind_speed_ms: Optional[float] = Field(None, description="Wind speed in meters per second")
    precipitation_mm: Optional[float] = Field(None, description="Precipitation/rainfall in millimeters")
...
class CurrentWeatherResponse(BaseModel):
    ...
    records: List[WeatherRecord] = Field(default_factory=list, description="Latest weather readings")


class WeatherTrendsResponse(BaseModel):
    city: str = Field(..., description="Target city for trends")
    records: List[WeatherRecord] = Field(default_factory=list, description="Historical time-series series")
