...
class TestExplainService:
    ...
    def test_headline_contains_metric_and_district(self):
        ...
        assert (
            "traffic" in result["headline"].lower()
            or "congestion" in result["headline"].lower()
        )
    ...
    def test_factor_confidence_in_range(self):
        ...
        for f in result["contributing_factors"]:
            ...
            ), f"Out of range confidence: {f['confidence']}"
    ...
    def test_correlate_factors_exist_for_power(self):
        ...
        assert (
            "CORRELATE" in types
        ), "POWER anomaly should have cross-domain CORRELATE factors"
    ...
    def test_unknown_metric_fallback(self):
        result = ExplainService.explain_anomaly(
            anomaly_id="T23",
            metric_name="mystery_sensor_x99",
            severity="WARNING",
            category="PUBLIC_INFRASTRUCTURE",
            score=0.55,
            timestamp="2026-05-28T10:00:00",
    ...
    def test_batch_cross_incident_patterns_list(self):
        ...
        assert (
            len(result["cross_incident_patterns"]) > 0
        )  # POWER+INTERNET pattern detected
        ...
        async def _run():
            async with AsyncClient(
                ...
                resp = await client.get(
                    "/api/v1/explain/preview?metric=water_quality&category=WATER&score=0.91&severity=CRITICAL"
                )
                assert resp.status_code == 200
                data = resp.json()
                assert (
                    "water" in data["headline"].lower()
                    or "quality" in data["headline"].lower()
                )
