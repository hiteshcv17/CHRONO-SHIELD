...
class TestReplayService:
    ...
    def test_bucket_structure(self):
        ...
        required = [
            "bucket_index", "timestamp_start", "timestamp_end", "label",
            "anomaly_count", "critical_count", "severity_distribution",
            "category_distribution", "peak_score", "health_delta", "event_ids"
        ]
    ...
    def test_bucket_category_distribution_structure(self):
        ...
        for bucket in data["buckets"]:
            ...
            for cat in ["POWER", "TRAFFIC", "WATER", "INTERNET", "PUBLIC_INFRASTRUCTURE"]:
                assert cat in cd
    ...
    def test_incident_record_structure(self):
        ...
        required = [
            "id", "timestamp", "metric_name", "severity", "category",
            "score", "description", "district", "acknowledged",
            "duration_minutes", "cascaded", "related_ids",
            "root_cause_hint", "resolution_hint", "bucket_index"
        ]
        for field in required:
    ...
    def test_incident_bucket_index_within_range(self):
        ...
        n_buckets = 48
        for inc in data["incidents"]:
            assert 0 <= inc["bucket_index"] < n_buckets, (
    ...
    def test_bucket_event_ids_match_incidents(self):
        ...
        for bucket in data["buckets"]:
            for eid in bucket["event_ids"]:
                assert eid in all_ids, f"Event ID {eid} in bucket not found in incidents"

    def test_replay_frame_structure(self):
        data = ReplayService.get_timeline(24)
        frame = ReplayService.get_replay_frame(10, data["incidents"])
        required = [
            "bucket", "active_incidents", "cumulative_critical",
            "cumulative_total", "system_health", "dominant_category", "alert_level"
        ]
        for field in required:
    ...
    def test_replay_frame_cumulative_monotonic(self):
        data = ReplayService.get_timeline(24)
        totals = [ReplayService.get_replay_frame(i, data["incidents"])["cumulative_total"] for i in [0, 10, 24, 47]]
        assert totals == sorted(totals), "Cumulative total should be monotonically non-decreasing"

    def test_compare_incidents_same(self):
        data = ReplayService.get_timeline(24)
        result = ReplayService.compare_incidents("INC-017", "INC-018", data["incidents"])
        assert result
    ...
    def test_compare_incidents_correlated(self):
        ...
        result = ReplayService.compare_incidents("INC-005", "INC-006", data["incidents"])
    ...
    def test_compare_incidents_combined_risk(self):
        data = ReplayService.get_timeline(24)
        result = ReplayService.compare_incidents("INC-032", "INC-030", data["incidents"])
        assert result["combined_risk"] in {"MODERATE", "HIGH", "EXTREME"}

    def test_compare_incidents_missing_returns_empty(self):
        data = ReplayService.get_timeline(24)
        result = ReplayService.compare_incidents("INC-999", "INC-001", data["incidents"])
        assert result == {}
...
class TestReplayAPI:
    ...
    def test_compare_endpoint_ok(self):
        async def _run():
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                resp = await client.get("/api/v1/replay/compare?id_a=INC-005&id_b=INC-006")
                assert resp.status_code == 200
                data = resp.json()
    ...
    def test_compare_endpoint_not_found(self):
        async def _run():
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
                resp = await client.get("/api/v1/replay/compare?id_a=INC-999&id_b=INC-001")
