...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if req_count > limit:
                logger.warning(
                    f"Rate limit breached for IP {client_ip} on Redis (requests: {req_count}/{limit})"
                )
        ...
        # 2. Enforce using In-Memory Fallback (Local resilient fallback)
            ...
            logger.debug(
                f"Redis rate limiting offline, falling back to local memory: {e}"
            )
            now = time.time()
            ...
            _IN_MEMORY_LIMITS[client_ip] = [
                t for t in _IN_MEMORY_LIMITS[client_ip] if now - t < 60
            ]

            req_count = len(_IN_MEMORY_LIMITS[client_ip])
            if req_count >= limit:
                logger.warning(
                    f"Rate limit breached for IP {client_ip} in-memory (requests: {req_count + 1}/{limit})"
                )
                err_envelope = ApiResponse.fail(
                    code="TOO_MANY_REQUESTS",
                    message="API rate limit exceeded (100 req/min). Please try again later.",
