...
class ForecastingService:
    ...
    async def get_telemetry_forecast(
        ...
        if len(timestamps) < 30:
            logger.info(
                "Aligned dataframe has insufficient data points. Synthesizing robust historical baseline..."
            )
            ...
            for i, ts in enumerate(timestamps):
                ...
                val_demand = (
                    45.0
                    + 18.0 * math.exp(-(((hour - 17.5) / 3.0) ** 2))
                    + 12.0 * math.exp(-(((hour - 8.5) / 2.0) ** 2))
                    + (6.0 if dow < 5 else -10.0)
                )
                # Traffic Congestion: Weekday rush hours, weekend noon hump
                val_jam = (
                    0.15
                    + 0.45 * math.exp(-(((hour - 8.0) / 1.5) ** 2))
                    + 0.5 * math.exp(-(((hour - 17.5) / 2.0) ** 2))
                )
                if dow >= 5:
        ...
        for _, row in future_forecast.iterrows():
            ...
            if val > critical_threshold or val > (hist_max + 1.2 * hist_std):
                ...
                if in_anomaly_window:
                    severity = (
                        "CRITICAL"
                        if max_val > (critical_threshold * 1.15)
                        else "WARNING"
                    )
        ...
        if in_anomaly_window:
            severity = (
                "CRITICAL" if max_val > (critical_threshold * 1.15) else "WARNING"
            )
            predicted_anomalies.append(
                {
                    "timestamp": start_ts,
