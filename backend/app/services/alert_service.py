...
class AlertRepository:
    ...
    @staticmethod
    async def get_active_cooldown_alert(
        db: Optional[AsyncSession], metric: str, now: datetime
    ) -> Optional[PrioritizedAlertRecord]:
        """Fetch alert for the metric that has active cooldown."""
        if db is None:
            cooldown_matches = [
                a
                for a in _MOCK_ALERTS_REGISTRY
        ...
        try:
            ...
            if settings.ENVIRONMENT != "production":
                logger.warning(
                    f"DB cooldown query failed in dev mode, falling back to mock: {e}"
                )
                cooldown_matches = [
                    a
                    for a in _MOCK_ALERTS_REGISTRY
    ...
    @staticmethod
    async def get_duplicate_alert(
        db: Optional[AsyncSession], metric: str, five_min_ago: datetime
    ) -> Optional[PrioritizedAlertRecord]:
        ...
        if db is None:
            dup_matches = [
                ...
                and a.last_occurrence >= five_min_ago
            ]
        ...
        try:
            ...
            if settings.ENVIRONMENT != "production":
                logger.warning(
                    f"DB duplicate check failed in dev, falling back to mock: {e}"
                )
                dup_matches = [
                    a
                    for a in _MOCK_ALERTS_REGISTRY
    ...
    async def get_alert_by_id(
        db: Optional[AsyncSession], alert_id: str
    ) -> Optional[PrioritizedAlertRecord]:
        ...
        stmt = select(PrioritizedAlertRecord).where(
            PrioritizedAlertRecord.id == alert_id
        )
        try:
            res = await db.execute(stmt)
            ...
            if settings.ENVIRONMENT != "production":
                logger.warning(
                    f"DB fetch by id failed in dev, falling back to mock: {e}"
                )
                matches = [x for x in _MOCK_ALERTS_REGISTRY if x.id == alert_id]
                return matches[0] if matches else None
            raise e
    ...
    @staticmethod
    async def get_breached_alerts(
        db: Optional[AsyncSession], sla_cutoff: datetime
    ) -> List[PrioritizedAlertRecord]:
        """Fetch active/escalated alerts breached since SLA cutoff that aren't escalated yet."""
        if db is None:
            return [
                a
                for a in _MOCK_ALERTS_REGISTRY
                if a.status in ["ACTIVE", "ESCALATED"]
                and a.timestamp <= sla_cutoff
        ...
        try:
            ...
            if settings.ENVIRONMENT != "production":
                logger.warning(
                    f"DB breached sweep failed in dev, falling back to mock: {e}"
                )
                return [
                    a
                    for a in _MOCK_ALERTS_REGISTRY
    ...
    async def query_alerts(
        ...
        if db is None:
            ...
            if status_filter:
                filtered = [
                    x for x in filtered if x.status.lower() == status_filter.lower()
                ]
            if severity_filter:
                filtered = [
                    x
                    for x in filtered
                    if x.current_severity.lower() == severity_filter.lower()
                ]
            sorted_records = sorted(
                filtered, key=lambda a: a.priority_score, reverse=True
            )
        ...
        if status_filter:
            filter_stmt = filter_stmt.where(
                PrioritizedAlertRecord.status.ilike(status_filter)
            )
        if severity_filter:
            filter_stmt = filter_stmt.where(
                PrioritizedAlertRecord.current_severity.ilike(severity_filter)
            )
        ...
        try:
            ...
            offset = (page - 1) * page_size
            paginated_stmt = (
                filter_stmt.order_by(PrioritizedAlertRecord.priority_score.desc())
                .offset(offset)
                .limit(page_size)
            )
            ...
            if settings.ENVIRONMENT != "production":
                logger.warning(
                    f"DB query_alerts failed in dev, falling back to mock: {e}"
                )
                filtered = _MOCK_ALERTS_REGISTRY
                if status_filter:
                    filtered = [
    ...
    @staticmethod
    async def save(
        db: Optional[AsyncSession], alert: PrioritizedAlertRecord
    ) -> PrioritizedAlertRecord:
...
class AlertPrioritizationEngine:
    ...
    def calculate_priority_score(
        severity: str, score: float, occurrences: int, escalation: int
    ) -> float:
        ...
        return max(
            0.0, min(100.0, base_sev + score_weight + dup_weight + escalation_weight)
        )

    @staticmethod
    async def inject_and_prioritize_anomaly(
        db: Optional[AsyncSession], payload: AnomalyCreate
    ) -> PrioritizedAlertRecord:
        ...
        try:
            active_cooldown_alert = await AlertRepository.get_active_cooldown_alert(
                db, metric, now
            )
        except Exception as e:
        ...
        if active_cooldown_alert:
            logger.info(
                f"Anomaly trigger '{payload.id}' suppressed due to active metric '{metric}' cooldown until {active_cooldown_alert.cooldown_until}"
            )
            suppressed_alert = PrioritizedAlertRecord(
                ...
                priority_score=AlertPrioritizationEngine.calculate_priority_score(
                    "LOW", anomaly_score, 1, 0
                ),
                status="SUPPRESSED",
                occurrence_count=1,
                timestamp=now,
        ...
        try:
            duplicate_alert = await AlertRepository.get_duplicate_alert(
                db, metric, five_min_ago
            )
        except Exception as e:
        ...
        if duplicate_alert:
            ...
            duplicate_alert.priority_score = (
                AlertPrioritizationEngine.calculate_priority_score(
            ...
            try:
                ...
                logger.info(
                    f"Incident '{payload.id}' de-duplicated onto alert '{duplicate_alert.id}' (count={duplicate_alert.occurrence_count})"
                )
        ...
        p_score = AlertPrioritizationEngine.calculate_priority_score(
            severity, anomaly_score, 1, 0
        )
        ...
        try:
            ...
            logger.info(
                f"Created prioritized alert '{new_alert.id}' for metric '{metric}' (score={p_score:.1f})"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to trigger notifications for new alert {new_alert.id}: {ne}"
                )
    ...
    async def get_prioritized_queue(
        ...
        try:
            ...
            for alert in breached_alerts:
                ...
                curr_idx = (
                    sev_hierarchy.index(alert.current_severity.upper())
                    if alert.current_severity.upper() in sev_hierarchy
                    else 1
                )
                ...
                try:
                    ...
                    await NotificationDeliveryService.trigger_notifications(
                        db, alert, status_override="ESCALATED"
                    )
                except Exception as ne:
                    logger.error(
                        f"Failed to trigger escalation notifications for alert {alert.id}: {ne}"
                    )
        ...
        try:
            return await AlertRepository.query_alerts(
                db, status_filter, severity_filter, page, page_size
            )
        except Exception as e:
    ...
    async def acknowledge_alert(
        ...
        try:
            ...
            await AlertRepository.commit(db)
            logger.info(
                f"Alert '{alert_id}' Acknowledged. Metric '{alert.metric_name}' in cooldown until {cooldown_limit}"
            )
            return alert
        except Exception as e:
            if db is not None:
    ...
    async def resolve_alert(
        ...
        try:
            ...
            await AlertRepository.commit(db)
            logger.info(
                f"Alert '{alert_id}' Resolved. Metric '{alert.metric_name}' in cooldown until {cooldown_limit}"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to trigger resolution notifications for alert {alert.id}: {ne}"
                )
