...
def _jitter_coords(base_lat: float, base_lng: float, seed: str, scale: float = 0.018) -> Tuple[float, float]:
...
class GeoService:
    ...
    _MOCK_ANOMALIES = [
        {"id": "geo-001", "metric_name": "energy_demand",      "severity": "CRITICAL", "score": 0.91, "description": "Grid instability detected — peak demand spike", "timestamp": "2026-05-28T12:01:00", "acknowledged": False},
        {"id": "geo-002", "metric_name": "traffic_jam",        "severity": "WARNING",  "score": 0.72, "description": "Severe congestion — East Industrial corridor", "timestamp": "2026-05-28T12:04:00", "acknowledged": False},
        {"id": "geo-003", "metric_name": "water_pressure",     "severity": "CRITICAL", "score": 0.88, "description": "Low pressure anomaly — South Harbor pipeline", "timestamp": "2026-05-28T12:07:00", "acknowledged": False},
        {"id": "geo-004", "metric_name": "network_latency",    "severity": "WARNING",  "score": 0.65, "description": "Elevated latency — North District CDN node",  "timestamp": "2026-05-28T12:09:00", "acknowledged": True},
        {"id": "geo-005", "metric_name": "cpu_usage",          "severity": "INFO",     "score": 0.45, "description": "High CPU utilization — API Gateway cluster",   "timestamp": "2026-05-28T12:12:00", "acknowledged": False},
        {"id": "geo-006", "metric_name": "road_signal",        "severity": "WARNING",  "score": 0.58, "description": "Signal failure — Central Grid intersection",   "timestamp": "2026-05-28T12:15:00", "acknowledged": False},
        {"id": "geo-007", "metric_name": "grid_voltage",       "severity": "CRITICAL", "score": 0.95, "description": "Critical voltage drop — West Residential zone","timestamp": "2026-05-28T12:18:00", "acknowledged": False},
        {"id": "geo-008", "metric_name": "flood_sensor",       "severity": "WARNING",  "score": 0.70, "description": "Rainfall accumulation — South Harbor basin",   "timestamp": "2026-05-28T12:21:00", "acknowledged": False},
        {"id": "geo-009", "metric_name": "vehicle_count",      "severity": "INFO",     "score": 0.38, "description": "Unusual vehicle density — East Industrial",    "timestamp": "2026-05-28T12:24:00", "acknowledged": True},
        {"id": "geo-010", "metric_name": "internet_bandwidth", "severity": "CRITICAL", "score": 0.82, "description": "Bandwidth saturation — North District backbone","timestamp": "2026-05-28T12:27:00", "acknowledged": False},
        {"id": "geo-011", "metric_name": "water_quality",      "severity": "WARNING",  "score": 0.60, "description": "Turbidity spike — Central water treatment",    "timestamp": "2026-05-28T12:30:00", "acknowledged": False},
        {"id": "geo-012", "metric_name": "power_outage",       "severity": "CRITICAL", "score": 0.98, "description": "Complete power outage — East Industrial block", "timestamp": "2026-05-28T12:33:00", "acknowledged": False},
        {"id": "geo-013", "metric_name": "traffic_accident",   "severity": "WARNING",  "score": 0.67, "description": "Multi-vehicle incident — North District highway","timestamp": "2026-05-28T12:36:00", "acknowledged": False},
        {"id": "geo-014", "metric_name": "infrastructure_defect","severity": "INFO",   "score": 0.42, "description": "Pothole report cluster — West Residential",    "timestamp": "2026-05-28T12:39:00", "acknowledged": True},
        {"id": "geo-015", "metric_name": "network_packet_loss","severity": "WARNING",  "score": 0.73, "description": "Packet loss spike — Central Grid fiber link",  "timestamp": "2026-05-28T12:42:00", "acknowledged": False},
    ]

    @staticmethod
    async def get_anomaly_points(db: Optional[AsyncSession] = None) -> List[Dict[str, Any]]:
        """
        ...
        if db is not None:
            try:
                ...
                logger.warning(f"DB anomaly query failed for geo projection: {e}. Using mock data.")
    ...
    async def get_region_statuses(
        ...
        anomaly_points: Optional[List[Dict]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        for zone in CITY_ZONES:
            ...
            dominant = max(cat_counts, key=cat_counts.get) if cat_counts else zone["primary_categories"][0]
            
            critical_count = len([p for p in zone_points if p["severity"] == "CRITICAL"])
    ...
    async def get_full_map(db: Optional[AsyncSession] = None) -> Dict[str, Any]:
        ...
        most_affected = min(regions, key=lambda r: r["health_score"]) if regions else None
