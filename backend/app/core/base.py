...
class ErrorDetail(BaseModel):
    ...
    field: Optional[str] = Field(None, description="Specific field that triggered the error (if applicable)")
...
class ApiResponse(BaseModel, Generic[T]):
    ...
    success: bool = Field(..., description="True if the request was processed successfully")
    data: Optional[T] = Field(None, description="Response payload (null on error)")
    error: Optional[ErrorDetail] = Field(None, description="Error detail (null on success)")
    timestamp: str = Field(
    ...
    @classmethod
    def fail(cls, code: str, message: str, trace_id: Optional[str] = None) -> "ApiResponse[None]":
        """Create an error response with the given code and message."""
        return cls(success=False, error=ErrorDetail(code=code, message=message, trace_id=trace_id))
